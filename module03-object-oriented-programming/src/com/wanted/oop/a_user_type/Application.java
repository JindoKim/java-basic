...
public class Application {
    public static void main(String[] args) {
        ...
        * 값들을 인자로 전달해야 하기 때문에 한 눈에 안들어온다.
        ...
        *
        * 사용자 정의의 자료형 사용해보기
        * 1. 클래스명 변수명 = new 클래스명(); new는 할당연산자이다(Heap이라는 공간에 공간을 만들것이다)
        * 지금까지 이렇게 작성한 구문은 객체를 생성하는 구문이다.
        * 2. 사용자 정의의 자료형인 클래스를 생성하면, 클래스에 우리가 정의한 필드(전역변수) , 메서드 대로 객체가 생성된다.
        */

        //Stack 에는 Member 자료형이 저장됌.
        Member member = new Member();
        /* 멤버가 가진 값에 접근하기 위해서는 인스턴스변수명.필드명
        * 여기서 . 은 참조연산자로서 레퍼런스 변수가 참조하고 있는 주소에 접근하겠다는 의미이다.
        * */
        System.out.println("member 가 가진 이름 : " + member.name); //Heap 메모리 값에는 빈 값이 존재할 수 없음.
        System.out.println("member 가 가진 이름 : " + member.age); // 그렇기에 null, 0 이 기본값으로 대체된다.
        // 필드에 접근해서 값을 초기화
        member.id = "user02";
        member.pwd = "pass02";
